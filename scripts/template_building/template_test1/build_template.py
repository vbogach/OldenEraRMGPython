...
from olden_era.utils import strip_roads, replace_ownership
...
spawn_side_template = Connection(
    name="", start="", end="", connectionType="Direct", road=True, simTurnSquad=True, guardValue=3000,
    guardWeeklyIncrement=0.20)
side_treasure_template = Connection(
    name="", start="", end="", connectionType="Direct", road=True, simTurnSquad=True, guardValue=9000,
    guardWeeklyIncrement=0.20)
treasure_supertreasure_template = Connection(
    name="", start="", end="", connectionType="Direct", road=True, simTurnSquad=True, guardValue=18000,
    guardWeeklyIncrement=0.20)
spawn_supertreasure_template = Connection(
    name="", start="", end="", connectionType="Direct", road=True, simTurnSquad=True, guardValue=48000,
    guardWeeklyIncrement=0.10)

player_indices = list(range(1, 4))
for player_idx in player_indices:
    spawn = (ZoneBuilder(spawn_template).
    ...
    side = (ZoneBuilder(side_template).
            remove_cities().
    ...
    treasure = (ZoneBuilder(treasure_template).
                remove_cities().
                remove_roads().
    ...
    connection_spawn_side = (ConnectionBuilder(spawn_side_template).
                             from_to(spawn, side).
                             build())
...
for p1, p2 in combinations(player_indices, 2):
    supertreasure_side = (ZoneBuilder(supertreasure_template).
                          remove_cities().
                          remove_roads().
                          set_name(f"SupertreasureSide{p1}{p2}").build())
    zones[supertreasure_side.name] = supertreasure_side

    spawn_p1 = zones[f"Spawn{p1}"]
    spawn_p2 = zones[f"Spawn{p2}"]
    supertreasure_mid = zones[f"SupertreasureMid"]
...
orientation = Orientation(
    zeroAngleZone="Spawn1", mode="MinimalBoundingSquare", baseAngleMin=120, baseAngleMax=120,
    randomAngleAmplitude=0, randomAngleStep=120)
...
with open(r"C:\Program Files (x86)\Steam\steamapps\common\Heroes of Might and Magic Olden Era\HeroesOldenEra_Data\StreamingAssets\map_templates\template.rmg.json", "w") as f:
